...
def main():
    ...
    df["id"] = range(len(df))
    # company/place are published columns as of the backfill in
    # backfill_company_place.py -- computed here too as a fallback so this
    # script still works standalone if that hasn't run yet.
    if "company" not in df.columns or "place" not in df.columns:
